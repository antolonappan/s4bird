...
class Efficency:
    ...
    def fit_null_r_V2(self,lmin=20,lmax=200):
        tensor = pk.load(open('../tensor.pkl','rb'))
        lensing = pk.load(open('../lensing.pkl','rb'))
        x = self.lib_pcl.ell
        sel = np.where((x >= lmin)& (x <= lmax))[0]
        ...
        beam = get_beam(30)
        noise = get_noise(2.15)
        
        def cl_theory(r,Alens):
            th = (r/0.1 * tensor) + (Alens * lensing)
            return th*beam**2 + noise
        
        len_m, len_s, del_m, del_s = self.get_stat
        
        #mean = del_m - self.bias - (lensing*beam**2 + noise)
        mean = del_m - self.bias
    
        mean2,std = self.spectrum_difference
        x = self.lib_pcl.ell
        var = std**2#np.sqrt(del_s**2 +self.bias_std**2)
        ...
        def chi_sq(theta):
            epsilon,r = theta
            #num = mean[sel] - (epsilon * (r*tensor[sel] - ref[sel]) )
            num = mean[sel] - (beam[sel]**2 + epsilon)*lensing[sel] - ((epsilon*r)*tensor[sel]) - noise[sel]
    ...
    def fit_null_r(self,lmin=20,lmax=200):
        tensor = pk.load(open('../tensor.pkl','rb'))
        mean,std = self.spectrum_difference
        x = self.lib_pcl.ell
        var = (std*10**6)**2
    ...
    def fit_r(self,lmin=20,lmax=100):
        tensor = pk.load(open('../tensor.pkl','rb'))
        lensing = pk.load(open('../lensing.pkl','rb'))
        x = self.lib_pcl.ell
        sel = np.where((x >= lmin)& (x <= lmax))[0]
        ...
        def chi_sq(theta):
            ...
            std = np.sqrt(del_s[sel]**2 + self.bias_std[sel]**2)
            return np.sum((del_m[sel] - self.bias[sel] - th[sel])**2/std**2)
        def chi_sq_new(theta):
            r,Alens = theta
            th = cl_theory(r,Alens)
            std = len_s[sel]
        ...
        pl.figure(figsize=(8,8))
        pl.plot(ell,cl_theory(res['x'][0],res['x'][1]),label='Best Fit')
        pl.errorbar(ell,del_m-self.bias,yerr=np.sqrt(del_s**2+self.bias_std**2),label='Data')
        pl.errorbar(ell,len_m,yerr=len_s,label='Lensed')
        pl.axhline(np.radians(2.15/60)**2)
        pl.axhline(np.radians(1.5/60)**2)
        pl.semilogy()
        pl.semilogx()
        pl.legend()
