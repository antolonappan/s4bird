...
class LH_base:
    ...
    def posterior(self,i):
        ...
        if os.path.isfile(fname) and self.cache:
            ...
            if self.fix_alens:
                pos = np.array([self.init[0]]) + 1e-4 * np.random.randn(100, 1)
            else:
...
class SampleCov:
    ...
    @property    
    def lensed_fid(self):
        fname = os.path.join(self.lib_dir,'C_len_fid.pkl')
    ...
    def delensed_fid(self):
        fname = os.path.join(self.lib_dir,'C_delen_fid.pkl')
        if os.path.isfile(fname):
            return pk.load(open(fname,'rb'))
        else:
    ...
    def lensed_fid_cov(self):
        fname = os.path.join(self.lib_dir,'Cov_len_fid.pkl')
        if os.path.isfile(fname):
            return pk.load(open(fname,'rb'))
        else:
    ...
    def delensed_fid_cov(self):
        if self.include_bias:
            ...
            fname = os.path.join(self.lib_dir,'Cov_delen_fid.pkl')
        if os.path.isfile(fname):
            return pk.load(open(fname,'rb'))
        else:
